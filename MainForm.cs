...
{
    // Minimal settings model - if you already have a CaptureSettings class in your project
    // you can remove this one and use the existing type.
...
n    {
    ...
    public int HotkeyModifiers { get; set; } = 0; // none
    public int HotkeyKey { get; set; } = (int)Keys.F9;
    public Rectangle? Region { get; set; }
}

// Lightweight session manager stub. Replace with your real implementation.
public static class SessionManager
{
    public static string? FindActiveSession(string root)
    {
        try
        {
            if (!Directory.Exists(root)) return null;
            // look for a folder named "active.session" or the most recent folder
    ...
    public record SessionInfo(int IntervalSeconds);

    public static SessionInfo? LoadSession(string folder)
    {
        try
        {
            var meta = Path.Combine(folder, "session.json");
            if (!File.Exists(meta)) return null;
            var json = File.ReadAllText(meta);
            var doc = JsonSerializer.Deserialize<JsonElement>(json);
            if (doc.TryGetProperty("IntervalSeconds", out var v) && v.TryGetInt32(out var i))
    ...
    public static string CreateNewSession(string root, int interval, int desiredSec)
    {
        Directory.CreateDirectory(root);
        var folder = Path.Combine(root, "session_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
        Directory.CreateDirectory(folder);
        var meta = new { IntervalSeconds = interval, DesiredSeconds = desiredSec, Created = DateTime.Now };
        File.WriteAllText(Path.Combine(folder, "session.json"), JsonSerializer.Serialize(meta));
        return folder;
    }

    public static void IncrementFrameCount(string folder)
    {
        try
        {
            var countFile = Path.Combine(folder, "count.txt");
            int c = 0;
            if (File.Exists(countFile)) int.TryParse(File.ReadAllText(countFile), out c);
            File.WriteAllText(countFile, (c + 1).ToString());
        }
        catch { }
    }
}

// Minimal ffmpeg runner stub to keep the project compiling. Replace with your real runner.
public static class FfmpegRunner
{
    public static async Task<(int exitCode, string error)> RunFfmpegAsync(string exe, string args)
    {
        try
        {
...
public partial class MainForm : Form
{
    ...
    // Implementation for missing functions referenced elsewhere in the code
    private void LoadSettings()
    {
        try
        {
            var path = Path.Combine(AppContext.BaseDirectory, "settings.json");
            if (!File.Exists(path)) { settings = new CaptureSettings(); return; }
            var json = File.ReadAllText(path);
            settings = JsonSerializer.Deserialize<CaptureSettings>(json) ?? new CaptureSettings();
        }
        catch
        {
    ...
    private void UpdateQualityControls()
    {
        SafeInvoke(() =>
        {
            if (trkQuality != null) trkQuality.Value = settings.JpegQuality;
            if (numQuality != null) numQuality.Value = settings.JpegQuality;
            trkQuality.Enabled = (cmbFormat?.SelectedItem?.ToString() ?? settings.Format) == "JPEG";
            numQuality.Enabled = trkQuality.Enabled;
        });
    }
